...
#数据导入
data = pd.read_csv('C:\\Users\\seldo\\Desktop\\dataofmatch\\Online_Retail.csv')
...
Cus_buy_times_df = data[data['Quantity']>0].groupby(['CustomerID','Description'])['InvoiceNo'].nunique()#nunique()#每个客户消费不同产品的次数
...
Back_buy_count_m = data[data['Quantity']>0].pivot_table(index=['CustomerID'],
                                                        columns=['Year_Month'],
                                                        values='InvoiceNo',
                                                        aggfunc='nunique').fillna(0)#count 表示计算每次购买的产品数，nunique 表示购买次数
...
def kline_base() -> pe.charts.Kline:
    K = (
        pe.charts.Kline()
            .add_xaxis(["2010/12","2011/1","2011/2","2011/3","2011/4","2011/5","2011/6","2011/7","2011/8","2011/9","2011/10","2011/11","2011/12"])
...
# 每天客单价
Customer_ev_d =data[data['Quantity']>0].groupby('Year_Month_Day')['Toa_Pri'].sum()/data[data['Quantity']>0].groupby('Year_Month_Day')['InvoiceDate'].nunique()
# 每周客单价
Customer_ev_w =data[data['Quantity']>0].groupby('Year_Month_Week')['Toa_Pri'].sum()/data[data['Quantity']>0].groupby('Year_Month_Week')['InvoiceDate'].nunique()
# 每月客单价
Customer_ev_m =data[data['Quantity']>0].groupby('Year_Month')['Toa_Pri'].sum()/data[data['Quantity']>0].groupby('Year_Month')['InvoiceDate'].nunique()
Customer_ev_d_tu = (
    pe.charts.Line()
...
Ret_rate_w = data[data['Quantity']<0].groupby('Year_Month_Week')['InvoiceNo'].nunique()/data[data['Quantity']>0].groupby('Year_Month_Week')['InvoiceNo'].nunique()
Ret_rate_m = data[data['Quantity']<0].groupby('Year_Month')['InvoiceNo'].nunique()/data[data['Quantity']>0].groupby('Year_Month')['InvoiceNo'].nunique()
#图
Ret_w_tu = (
    pe.charts.Line()
...
Customer_class_count2['Rate'] = (Customer_class_count2['Customer_class_count']/Customer_class_count2['Customer_class_count'].sum())*100
#图
...
Customer_class_rate_tu = (
    pe.charts.Pie()
        .add(
        "",
        [list(z) for z in zip(Customer_class_count2['Customer_class_name'].tolist(),Customer_class_count2['Rate'].astype('int').tolist())],
        radius=["40%", "75%"],
    )
        .set_global_opts(
...
#客户分析
Customer_anyl =  data[data['Quantity']>0].groupby('CustomerID').agg({'InvoiceNo':'nunique', 'Quantity':'sum', 'Toa_Pri':'sum'}).describe()
#产品的分析
Product_anyl = data[data['Quantity']>0].groupby('StockCode').agg({'InvoiceNo':'nunique', 'Quantity':'sum','Toa_Pri':'sum'}).describe()



#消费周期
Buy_interval = data[data['Quantity']>0].groupby('CustomerID').apply(lambda day: day['InvoiceDate'] - day['InvoiceDate'].shift())
